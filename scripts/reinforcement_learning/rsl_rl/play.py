...
def main():
    ...
    if enable_cameras:
        ...
        rep_writer = rep.BasicWriter(
            ...
            frame_padding=0,
            colorize_instance_id_segmentation=env.unwrapped.scene["camera_sensor"].cfg.colorize_instance_id_segmentation,
            colorize_instance_segmentation=env.unwrapped.scene["camera_sensor"].cfg.colorize_instance_segmentation,
            colorize_semantic_segmentation=env.unwrapped.scene["camera_sensor"].cfg.colorize_semantic_segmentation,
        )
    ...
    while simulation_app.is_running():
        ...
        if enable_cameras:
            ...
            single_cam_data = convert_dict_to_backend(
                {k: v[0] for k, v in env.unwrapped.scene["camera_sensor"].data.output.items()}, backend="numpy"
            )
            rep_output = {"annotators":{}}
            for key, data in zip(single_cam_data.keys(), single_cam_data.values()):
                rep_output["annotators"][key] = {"render_product":{"data":data}}

            rep_output["trigger_outputs"] = {"on_time": env.unwrapped.scene["camera_sensor"].frame[0]}
