...
class H1RoughDemo:
    ...
    * ESC: exit current third-person view"""
    ...
    def __init__(self):
        ...
        env_cfg.scene.camera_sensor = TiledCameraCfg(
            prim_path="{ENV_REGEX_NS}/Robot/torso_link/camera",
            update_period=0.0,
            height=1080,
            width=1920,
...
def main():
    ...
    camera_sensor_scene = demo_h1.env.unwrapped.scene["camera_sensor"]

    output_dir = osp.join(osp.dirname(osp.realpath(__file__)), "output", "camera")
    rep_writer = rep.BasicWriter(
        output_dir=output_dir,
        frame_padding=0,
        colorize_instance_id_segmentation=camera_sensor_scene.cfg.colorize_instance_id_segmentation,
        colorize_instance_segmentation=camera_sensor_scene.cfg.colorize_instance_segmentation,
        colorize_semantic_segmentation=camera_sensor_scene.cfg.colorize_semantic_segmentation,
    )
    camera_idx = 1
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            print("-------------------------------")
            print(camera_sensor_scene)
            print("Received shape of rgb   image: ", camera_sensor_scene.data.output["rgb"].shape)
            print("Received shape of depth image: ", camera_sensor_scene.data.output["distance_to_image_plane"].shape)
            print("Received shape of depth image: ", camera_sensor_scene.data.output["distance_to_camera"].shape)
            print("-------------------------------")

            single_cam_data = convert_dict_to_backend(
                {k: v[camera_idx] for k, v in camera_sensor_scene.data.output.items()}, backend="numpy"
            )
            rep_output = {"annotators":{}}
            for key, data in zip(single_cam_data.keys(), single_cam_data.values()):
                rep_output["annotators"][key] = {"render_product":{"data":data}}

            rep_output["trigger_outputs"] = {"on_time": camera_sensor_scene.frame[camera_idx]}
